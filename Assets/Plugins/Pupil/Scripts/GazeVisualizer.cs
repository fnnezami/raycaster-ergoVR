...
namespace PupilLabs
{
    public class GazeVisualizer : MonoBehaviour
    {
        ...
        [Header("Settings")] [Range(0f, 1f)] public float confidenceThreshold = 0.6f;

        [Header("Projected Visualization")] public Transform projectionMarker;
        [Range(0.01f, 0.1f)] public float sphereCastRadius = 0.05f;

        public TMP_Text hitText;

        public GazeListener GazeListener { get; private set; }
        ...
        void ShowProjected()
        {
            ...
            if (Physics.SphereCast(origin, sphereCastRadius, direction, out RaycastHit hit, Mathf.Infinity))
            {
                ...
                hitText.text = hit.collider.name;
                ...
                {
                    projectionMarker.localScale =
                        GetErrorAngleBasedScale(origMarkerScale, hit.distance, angleErrorEstimate);
                }
            }
            else
            {
